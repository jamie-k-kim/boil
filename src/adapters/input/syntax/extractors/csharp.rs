...
impl SymbolExtractor for CSharpExtractor {
    fn analyze(&self, path: &Path, source: &str, tree: &Tree, token_count: usize) -> FileInfo {
        ...
        let import_query_str = "(using_directive) @import";
        static IMPORT_QUERY: OnceLock<tree_sitter::Query> = OnceLock::new();
        let import_query = IMPORT_QUERY.get_or_init(|| {
            tree_sitter::Query::new(tree_sitter_c_sharp::language(), import_query_str).unwrap()
        });
        if true {
        ...
        let symbol_query_str = "
            ...
            (field_declaration (variable_declaration (variable_declarator (identifier) @name))) @var
        ";

        static SYMBOL_QUERY: OnceLock<tree_sitter::Query> = OnceLock::new();
        let query = SYMBOL_QUERY.get_or_init(|| {
            tree_sitter::Query::new(tree_sitter_c_sharp::language(), symbol_query_str).unwrap()
        });
        if true {
            let mut cursor = tree_sitter::QueryCursor::new();
        ...
        let ref_query = REF_QUERY.get_or_init(|| {
            tree_sitter::Query::new(tree_sitter_c_sharp::language(), ref_query_str).unwrap()
        });
        if true {
            let mut cursor = tree_sitter::QueryCursor::new();
